...
%     end
    ...
    for roi = 1:ROIsPerImg
        ...
        ImgVol_o=ImgVol;
        loadROI_o=loadedROI;
        % RESCALE if your images are WAY too big PSBasran
        ImgVol = imresize3(ImgVol,Rescale);
        loadedROI = logical(ceil(imresize3(loadedROI,Rescale,'nearest')));
        VoxelSizeInfo_o = VoxelSizeInfo / Rescale;
        
        VoxelSizeInfo_o = VoxelSizeInfo_o / 10; % TO ACCOUNT FOR THE NEED TO INCREASE THE VOXEL SIZES IN EXCEL SHEETS
        
        
        %% Calculating features
        disp(['Calculating features for ROI # ',int2str(roi)]);
        F_all = Calc_All_Features(ImgVol,loadedROI , VoxelSizeInfo_o(casenum ,1) , VoxelSizeInfo_o(casenum ,2), isotVoxSize , isotVoxSize2D , qntz, BinSize, DiscType, DataType, VoxInterp, ROIInterp, ROI_PV, isIsot2D, isScale, isGLround, isReSegRng, ReSegIntrvl,isQuntzStat,IVHconfig,isOutliers,Feats2out); 
...
%% Saving the results
time    = now; 
str     = datestr(time,0); str(str==':')='_'; str(str==' ')='_';
if ifSave
    save([SafePathFln, dbsName,'__',int2str(NumOKfolders),'_cases_Isotrop_',num2str(isotVoxSize),'mm_',int2str(numel(BinSize)),'_bins_',DiscType,'_Discr___',str,'.mat'] ,...
